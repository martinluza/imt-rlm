...
def count_chickens(text):
   return text.count("chicken")
print(count_chickens(context_data))
'''
...
'''python
import re
matches = re.findall(r'\bchicken\b', context_data)
print(f"Found {len(matches)} occurrences of the word 'chicken'")
'''
...
context_data = context_data[:18000] # We take only the first 18000 characters
context_data = context_data[:8999 ] + "Marcelo has 4 infinity stones. " + context_data[8999:]
...
messages = [
  {
    'role': 'user',
    'content': f"{REPL_SYSTEM_PROMPT}. Remember that the name of the context variable is 'context_data' and it's already defined, DO NOT WRITE YOU'RE OWN VARIABLE. Now, please answer the following query based on the context: How many infinity stones does Marcelo have?"
  }
]
